...
export default function StepAddress({ data, onUpdate, onNext, onBack }: StepAddressProps) {
  const [zipCode, setZipCode] = useState(data.zipCode || "");
  const [address, setAddress] = useState(data.address || "");
  const [city, setCity] = useState(data.city || "");
  const [state, setState] = useState(data.state || "");

  const handleAddressFound = (foundAddress: { street: string; city: string; state: string; neighborhood?: string }) => {
